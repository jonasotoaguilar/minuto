-- Migration: fix_attendance_history_rpc_default_period
-- Purpose:
-- 1) Keep get_attendance_history_page RPC discoverable with the expected signature
-- 2) Default filter to current org month when year/month are omitted
-- 3) Tighten membership+organization authorization check

DROP FUNCTION IF EXISTS public.get_attendance_history_page(uuid, uuid, integer, integer);
