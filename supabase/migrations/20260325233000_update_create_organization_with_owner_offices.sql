-- Migration: update_create_organization_with_owner_offices
-- Extends transactional organization creation so every org gets a Remote office
-- and can optionally create one physical office from map coordinates.
...
BEGIN
  ...
  v_location := NULLIF(
    regexp_replace(trim(coalesce(p_location, '')), '\s+', ' ', 'g'),
    ''
  );
  IF v_location IS NOT NULL THEN
    IF length(v_location) < 3 OR length(v_location) > 180 THEN
      RAISE EXCEPTION 'Organization address length must be between 3 and 180 characters';
    END IF;
    IF v_location ~ '[<>]' THEN
      RAISE EXCEPTION 'Organization address contains invalid characters';
    END IF;
  END IF;
  ...
  IF p_office_latitude IS NOT NULL THEN
    ...
    v_office_address_label := NULLIF(
      regexp_replace(
        trim(coalesce(p_office_address_label, v_location, '')),
        '\s+',
        ' ',
        'g'
      ),
